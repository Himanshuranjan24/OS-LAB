...
int find_shortest_job(struct process p[], int n, int current_time) {
    int i, shortest_job = -1;
    int shortest_time = __INT_MAX__;
    for(i=0; i<n; i++) {
        if(p[i].arrival_time <= current_time && !p[i].completed && p[i].remaining_time < shortest_time) {
            shortest_job = i;
...
void sjf_scheduling(struct process p[], int n) {
    ...
    while(completed_jobs < n) {
        int job_index = find_shortest_job(p, n, current_time);
        if(job_index == -1) {
            current_time++;
            continue;
        }
        struct process *job = &p[job_index];
